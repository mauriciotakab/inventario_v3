                        ...
                        <div class="productos-form-field">
                            ...
                            <select id="tipo" name="tipo" required>
                                <?php foreach ($tiposProducto as $tipo): ?>
                                    <option value="<?= $tipo ?>" <?= (($_POST['tipo'] ?? '') === $tipo) ? 'selected' : '' ?>><?= $tipo ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="productos-form-field">
                            ...
                            <select id="estado" name="estado" required>
                                <?php foreach ($estadosProducto as $estado): ?>
                                    <option value="<?= $estado ?>" <?= (($_POST['estado'] ?? '') === $estado) ? 'selected' : '' ?>><?= $estado ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="productos-form-field">
                            ...
                            <select id="categoria_id" name="categoria_id" required>
                                ...
                                <?php foreach ($categorias as $categoria): ?>
                                    <option value="<?= $categoria['id'] ?>" <?= (($_POST['categoria_id'] ?? '') == $categoria['id']) ? 'selected' : '' ?>><?= htmlspecialchars($categoria['nombre']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="productos-form-field">
                            ...
                            <select id="proveedor_id" name="proveedor_id" required>
                                ...
                                <?php foreach ($proveedores as $proveedor): ?>
                                    <option value="<?= $proveedor['id'] ?>" <?= (($_POST['proveedor_id'] ?? '') == $proveedor['id']) ? 'selected' : '' ?>><?= htmlspecialchars($proveedor['nombre']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="productos-form-field">
                            <label for="tags">Tags</label>
                            <input type="text" id="tags" name="tags" value="<?= htmlspecialchars($_POST['tags'] ?? '') ?>" placeholder="Palabras clave para búsquedas">
                        </div>
                        <div class="productos-form-field">
                            <label for="clase_categoria">Clase/Categoría interna</label>
                            <input type="text" id="clase_categoria" name="clase_categoria" value="<?= htmlspecialchars($_POST['clase_categoria'] ?? '') ?>">
                        </div>
                    </div>
                    ...
                    <div class="productos-form-section">
                        ...
                        <div class="productos-form-grid">
                            <div class="productos-form-field" style="grid-column: span 2;">
                                ...
                                <textarea id="descripcion" name="descripcion" placeholder="Describe el producto o su uso principal"><?= htmlspecialchars($_POST['descripcion'] ?? '') ?></textarea>
                            </div>
                            <div class="productos-form-field" style="grid-column: span 2;">
                                <label for="especificaciones_tecnicas">Especificaciones técnicas</label>
                                <textarea id="especificaciones_tecnicas" name="especificaciones_tecnicas" placeholder="Voltaje, materiales, certificaciones, etc."><?= htmlspecialchars($_POST['especificaciones_tecnicas'] ?? '') ?></textarea>
                            </div>
                            <div class="productos-form-field">
                                <label for="marca">Marca</label>
                                <input type="text" id="marca" name="marca" value="<?= htmlspecialchars($_POST['marca'] ?? '') ?>">
                            </div>
                            <div class="productos-form-field">
                            ...
                            <div class="productos-form-field">
                                ...
                                <input type="text" id="forma" name="forma" value="<?= htmlspecialchars($_POST['forma'] ?? '') ?>">
                            </div>
                            <div class="productos-form-field">
                                <label for="origen">País de origen</label>
                                <input type="text" id="origen" name="origen" value="<?= htmlspecialchars($_POST['origen'] ?? '') ?>">
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section class="productos-form-card">
                    ...
                    <div class="productos-form-grid">
                        <div class="productos-form-field">
                            ...
                            <input type="number" step="0.01" id="peso" name="peso" value="<?= htmlspecialchars($_POST['peso'] ?? '') ?>" placeholder="Ej. 1.25">
                        </div>
                        <div class="productos-form-field">
                            <label for="ancho">Ancho (cm)</label>
                            <input type="number" step="0.01" id="ancho" name="ancho" value="<?= htmlspecialchars($_POST['ancho'] ?? '') ?>">
                        </div>
                        <div class="productos-form-field">
                            <label for="alto">Alto (cm)</label>
                            <input type="number" step="0.01" id="alto" name="alto" value="<?= htmlspecialchars($_POST['alto'] ?? '') ?>">
                        </div>
                        <div class="productos-form-field">
                            <label for="profundidad">Profundidad (cm)</label>
                            <input type="number" step="0.01" id="profundidad" name="profundidad" value="<?= htmlspecialchars($_POST['profundidad'] ?? '') ?>">
                        </div>
                        <div class="productos-form-field">
                            ...
                            <select id="unidad_medida_id" name="unidad_medida_id">
                                ...
                                <?php foreach ($unidades as $unidad): ?>
                                    <option value="<?= $unidad['id'] ?>" <?= (($_POST['unidad_medida_id'] ?? '') == $unidad['id']) ? 'selected' : '' ?>><?= htmlspecialchars($unidad['nombre']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="productos-form-field">
                            ...
                            <select id="almacen_id" name="almacen_id" required>
                                ...
                                <?php foreach ($almacenes as $almacen): ?>
                                    <option value="<?= $almacen['id'] ?>" <?= (($_POST['almacen_id'] ?? '') == $almacen['id']) ? 'selected' : '' ?>><?= htmlspecialchars($almacen['nombre']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="productos-form-field">
                            <label for="stock_actual">Stock actual *</label>
                            <input type="number" id="stock_actual" name="stock_actual" min="0" value="<?= htmlspecialchars($_POST['stock_actual'] ?? '0') ?>" required>
                        </div>
                        <div class="productos-form-field">
                            <label for="stock_minimo">Stock mínimo *</label>
                            <input type="number" id="stock_minimo" name="stock_minimo" min="0" value="<?= htmlspecialchars($_POST['stock_minimo'] ?? '0') ?>" required>
                        </div>
                        <div class="productos-form-field">
                            <label for="costo_compra">Costo de compra (MXN)</label>
                            <input type="number" step="0.01" id="costo_compra" name="costo_compra" value="<?= htmlspecialchars($_POST['costo_compra'] ?? '') ?>">
                        </div>
                        <div class="productos-form-field">
                            <label for="precio_venta">Precio de venta (MXN)</label>
                            <input type="number" step="0.01" id="precio_venta" name="precio_venta" value="<?= htmlspecialchars($_POST['precio_venta'] ?? '') ?>">
                        </div>
                    </div>
                </section>
                ...
                <section class="productos-form-card">
                    ...
                    <div class="productos-form-grid">
                        <div class="productos-form-field">
                            ...
                            <span class="productos-form-note">Formatos permitidos: JPG, PNG, WEBP. Tamaño máximo 2MB.</span>
                        </div>
                    </div>
                </section>
